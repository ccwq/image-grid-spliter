...
const planForGrid = (preset: GridPreset): SlicePlan => ({ horizontalLines: createEvenDividerLines(preset.rows), verticalLines: createEvenDividerLines(preset.cols), padding: 0, paddingUnit: 'percent', trimOuterEdges: false })
...
const {
  ...
  onFileChange, onDrop, onDragOver, onDragLeave, handleGlobalDragOver, handleGlobalDrop, requestDownloadPermission, triggerFileSelect,
  setAutoDownload, downloadSingleImage,
...
onMounted(() => { setDocumentLang(locale.value); restorePreferences(); requestDownloadPermission(); syncMobileFlag(); window.addEventListener('dragover', handleGlobalDragOver); window.addEventListener('drop', handleGlobalDrop); window.addEventListener('resize', syncMobileFlag); window.addEventListener('popstate', handleBackNavigation) })
...
<template>
  <main class="page">
    ...
    <GridPresetsPanel :tr="tr" :icons="icons" :presets="gridPresets" :visible-presets="visiblePresets" :selected-preset="selectedPreset" :show-preset-toggle="showPresetToggle" :preset-expanded="presetExpanded" v-model:custom-rows="customRows" v-model:custom-cols="customCols" :processing="state.processing" :is-mobile="isMobile" @select-preset="handleSelectPreset" @toggle-presets="togglePresetExpanded" @apply-custom-grid="applyCustomGridWithValidation" />
    <SliceEditor :image-src="images[0]?.objectUrl" :image-alt="images[0]?.baseName" :horizontal-lines="slicePlan.horizontalLines.map((line) => line * 100)" :vertical-lines="slicePlan.verticalLines.map((line) => line * 100)" :padding="slicePlan.padding" :padding-unit="slicePlan.paddingUnit" :include-outer="slicePlan.trimOuterEdges" :expanded="editorExpanded" :disabled="state.processing" @update:expanded="editorExpanded = $event" @update:horizontal-lines="updateHorizontalLines" @update:vertical-lines="updateVerticalLines" @update:padding="updateSlicePlan({ padding: $event })" @update:padding-unit="updateSlicePlan({ paddingUnit: $event })" @update:include-outer="updateSlicePlan({ trimOuterEdges: $event })" />
    <ExportSettings :tr="tr" :icons="icons" :export-format="exportFormat" :is-jpg-format="isJpgFormat" :quality-label="qualityLabel" :jpg-quality-percent="jpgQualityPercent" :processing="state.processing" @update:export-format="handleExportFormatChange" @update:jpg-quality-percent="jpgQualityPercent = $event" />
    ...
    <ResultsPanel :tr="tr" :icons="icons" :images="images" :tiles-heading="tilesHeading" :results-summary="resultsSummary" :auto-download="autoDownload" :processing="state.processing" :collapsed="resultsCollapsed" @toggle-collapsed="resultsCollapsed = !resultsCollapsed" @toggle-auto-download="handleAutoDownloadToggle" @trigger-downloads="triggerDownloads" @download-image="downloadSingleImage" @preview-tile="openPreview" @preview-all="() => openPreview()" />
  </main>
</template>
